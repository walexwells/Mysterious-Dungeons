...
import { createElementWithConnectionEvents } from "./createElementWithConnectionEvents";
...
suite.each(tagNames)(
  `${createElementWithConnectionEvents.name}: %s`,
  (tagName) => {
    ...
    beforeEach(() => {
      element = createElementWithConnectionEvents(tagName);
      element.addEventListener(ConnectionEvent.CONNECT, connectListener);
      element.addEventListener(ConnectionEvent.DISCONNECT, disconnectListener);
      clearMocks();
    });
